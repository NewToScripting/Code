    ...
    public class RatingItem : ButtonBase, IUpdateVisualState
    {
        ...
        private void OnDisplayValueChanged(double oldValue, double newValue)
        {
            if (!_settingDisplayValue)
            {
                ...
                throw new InvalidOperationException(string.Format(CultureInfo.CurrentUICulture, "Invalid attempt to change read-only property {0}.", "DisplayValue"));
            }
            else
            {
                if (newValue <= 0.0)
        ...
        protected virtual void OnIsReadOnlyChanged(bool oldValue, bool newValue)
        {
            if (!_settingIsReadOnly)
            {
                ...
                throw new InvalidOperationException(string.Format(CultureInfo.CurrentUICulture, "Invalid attempt to change read-only property {0}.", "IsReadOnly"));
            }
            else
            {
