...
namespace DisplayServiceInstallerClass
{
    public class InstallerHelper
    {
       ...
       /// <returns></returns>
        ...
        /// <summary>
        /// Attach Inspire database
        /// </summary>
        /// <param name="sInstance"></param>
        /// <param name="sUser"></param>
        /// <param name="sPassword"></param>
        /// <param name="sDBName"></param>
        /// <param name="sMDFFile"></param>
        public static bool AttachDB(string sInstance, string sUser, string sPassword, string sDBName, string sMDFFile, string logFile, string fileStreamFolder)
        {
            
            StringCollection cFiles = new StringCollection();
            try
            {                 
                if (System.IO.File.Exists(sMDFFile))
                {
                    cFiles.Add(sMDFFile);
                    ServerConnection oConn = new ServerConnection(sInstance, sUser, sPassword);
                    Server oServer = new Server(oConn);

                    AddFileSecurity(fileStreamFolder, oServer.SqlDomainGroup, FileSystemRights.FullControl, AccessControlType.Allow);
                    AddFileSecurity(sMDFFile, oServer.SqlDomainGroup, FileSystemRights.FullControl, AccessControlType.Allow);
                    AddFileSecurity(sMDFFile, "OWNER RIGHTS", FileSystemRights.FullControl, AccessControlType.Allow);
                    AddFileSecurity(logFile, oServer.SqlDomainGroup, FileSystemRights.FullControl, AccessControlType.Allow);
                    AddFileSecurity(logFile, "OWNER RIGHTS", FileSystemRights.FullControl, AccessControlType.Allow);

                    DirectoryInfo dir = new DirectoryInfo(fileStreamFolder);
                    RecursiveAddAccount(dir, oServer.SqlDomainGroup);
                    oServer.AttachDatabase(sDBName, cFiles, AttachOptions.RebuildLog);


                    Login inspireLogin = new Login();
                    inspireLogin.Name = "Inspire";

                    inspireLogin.Parent = oServer;
                    inspireLogin.LoginType = Microsoft.SqlServer.Management.Smo.LoginType.SqlLogin;
                    inspireLogin.PasswordExpirationEnabled = false;
                    inspireLogin.PasswordPolicyEnforced = false;
                    inspireLogin.DefaultDatabase = "InspireServerDB";
                    inspireLogin.Create("!nspire8");

                    inspireLogin.AddToRole("sysadmin");

                    Database db = oServer.Databases["InspireServerDB"];

                    User u = new User(db, @"Inspire");
                    u.Login = inspireLogin.Name;
        ...
        public static void RecursiveAddAccount(DirectoryInfo source, string user)
        {
            try
            {
                ...
                {
                    AddFileSecurity(fi.FullName, user, FileSystemRights.FullControl, AccessControlType.Allow); 
                }
                // Copy each subdirectory using recursion.
                foreach (DirectoryInfo di in source.GetDirectories())
                {
                    AddDirectorySecurity(di.FullName, user, FileSystemRights.FullControl, AccessControlType.Allow);
                    RecursiveAddAccount(di, user);
                }
            }
            catch 
            {
       ...
       /// <param name="sDBName"></param>
       /// <returns></returns>
        public static bool DetachDB(string sInstance, string sUser, string sPassword, string sDBName)
        {
            try
            {            
                ServerConnection oConn = new ServerConnection(sInstance, sUser, sPassword);
                Server oServer = new Server(oConn);
                oServer.DetachDatabase(sDBName, false);
                return true;
            }
            catch
            {
