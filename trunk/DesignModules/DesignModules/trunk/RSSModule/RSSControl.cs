...
namespace RSSModule
{
    public class RSSControl : ItemsControl
    {
        static RSSControl()
        {
            DefaultStyleKeyProperty.OverrideMetadata(
                typeof(RSSControl),
                new FrameworkPropertyMetadata(typeof(RSSControl)));
        ...
        public RSSControl()
        {
            m_commandItemsRO = new ReadOnlyObservableCollection<RSSCommandItem>(m_commandItems);
        ...
        public ReadOnlyObservableCollection<RSSCommandItem> Commands
        {
            get
            {
        ...
        public static readonly DependencyProperty CommandItemTemplateProperty =
            DependencyProperty.Register("CommandItemTemplate", typeof(DataTemplate), typeof(RSSControl));
        ...
        public static readonly DependencyProperty CommandItemTemplateSelectorProperty =
            DependencyProperty.Register("CommandItemTemplateSelector", typeof(DataTemplateSelector), typeof(RSSControl));
        ...
        private static readonly DependencyPropertyKey ItemCountPropertyKey =
            ...
            typeof(int), typeof(RSSControl), new PropertyMetadata(0));
        ...
        public static readonly DependencyProperty CurrentItemIndexProperty =
            DependencyProperty.Register("CurrentItemIndex", typeof(int), typeof(RSSControl),
        ...
        public static RoutedEvent CurrentItemIndexChangedEvent =
            ...
            typeof(RoutedPropertyChangedEventHandler<int>), typeof(RSSControl));
        ...
        {
            RSSControl RSSScroller = (RSSControl)element;
            RSSScroller.OnCurrentItemIndexChanged((int)e.OldValue, (int)e.NewValue);
        }
        ...
        private void resetCommands()
        {
            ...
            if (parentItemsCount != m_commandItems.Count)
            {
                if (parentItemsCount > m_commandItems.Count)
                {
                    ...
                    {
                        m_commandItems.Add(new RSSCommandItem(this, i));
                    }
                }
                else
                {
            ...
            {
                Debug.Assert(((RSSCommandItem)m_commandItems[i]).Index == i);
            }
#endif
        }

        private void findRSSDecorator()
        {
            if (this.Template != null)
            {
                RSSDecorator temp = this.Template.FindName(PART_RSSDecorator, this) as RSSDecorator;
                if (m_RSSDecorator != temp)
                {
                    m_RSSDecorator = temp;
                    if (m_RSSDecorator != null)
                    {
                        Binding binding = new Binding("CurrentItemIndex");
                        binding.Source = this;
                        m_RSSDecorator.SetBinding(RSSDecorator.TargetIndexProperty, binding);
                    }
                }
                else
                {
                    Debug.WriteLine("No element with name '" + PART_RSSDecorator + "' in the template.");
                }
            }
            else
            {
                Debug.WriteLine("No template defined for RSSScroller.");
            }
        }
        ...
        private RSSDecorator m_RSSDecorator;
        ...
        private readonly ObservableCollection<RSSCommandItem> m_commandItems = new ObservableCollection<RSSCommandItem>();
        private readonly ReadOnlyObservableCollection<RSSCommandItem> m_commandItemsRO;

        #endregion

        public const string PART_RSSDecorator = "PART_RSSDecorator";
    }
}
